...
In this use case, we will align **immunofluorescence (IF)** of the **Xenium In Situ** platform to an **H&E images** generated from the same sections as the Xenium. VoltRon provides multiple approaches for this alignment:

1. **Automated workflow** (recommended): Using the `addXeniumHE()` function for one-step automated alignment
2. **Interactive workflow**: Using `registerSpatialData()` with the Shiny interface for manual control
...
VoltRon provides the `addXeniumHE()` function to automatically align and add H&E images to Xenium data in a single step. This function handles the entire workflow automatically:

```{r eval = FALSE, class.source="watch-out"}
# Add H&E image with automated alignment - one simple step!
# Supports TIFF, PNG, JPEG, SVS (whole slide), and other formats
Xen_R1 <- addXeniumHE(
  Xen_R1,
  "Xenium_FFPE_Human_Breast_Cancer_Rep1_he_image.tif",
  channel_name = "H&E",
  invert_dapi = TRUE,      # Recommended for better DAPI-to-H&E alignment
  MAX_FEATURES = 500,      # Number of features to detect
  GOOD_MATCH_PERCENT = 0.15,  # Match quality threshold
  matcher = "FLANN",       # Fast matching algorithm
  method = "Homography"    # Transformation method
)

# For SVS format (whole slide images):
# Xen_R1 <- addXeniumHE(Xen_R1, "he_image.svs")
# The function auto-detects SVS and selects appropriate resolution
```

The function automatically:

* Loads the H&E image
* Extracts the DAPI channel from Xenium data
* Performs automated feature-based registration using OpenCV
* Warps the H&E image to align with Xenium coordinates
* Adds the aligned H&E as a new channel

You can now view the aligned H&E image:

```{r eval = FALSE, class.source="watch-out"}
# View aligned H&E
vrImages(Xen_R1, channel = "H&E")

# Check available channels
vrImageChannelNames(Xen_R1)
```

And use it in spatial visualization:

```{r eval = FALSE, class.source="watch-out"}
# Visualize gene expression on H&E background
vrSpatialFeaturePlot(Xen_R1, features = "ERBB2", channel = "H&E")
```

**Advanced Usage:** You can save and reuse alignment parameters for reproducible analysis:

```{r eval = FALSE, class.source="watch-out"}
# Create and save parameter set
...
# Reuse parameters on other samples
he_params <- readRDS("he_alignment_params.rds")
Xen_R2 <- do.call(addXeniumHE, c(
  list(Xen_R2, "Xenium_R2_he_image.tif"),
  he_params
))
```
...
For cases requiring manual control, VoltRon provides an interactive Shiny-based workflow. First, import the H&E image as a separate VoltRon object:
...
#### Interactive Automated Alignment

Now we align the H&E image to the DAPI image of the Xenium replicate using the **registerSpatialData** function. This launches an interactive Shiny application where you can adjust parameters and visualize alignment quality. Similar to the first use case, we need to negate the DAPI image to better match it with the H&E image.
...
After alignment, we manually extract the registered H&E image and add it as a new channel to the Xenium data. The spatial key of the registered H&E image will be **main_reg**.

```{r eval = FALSE, class.source="watch-out"}
# Extract registered H&E image
Xen_R1_image_reg <- xen_reg$registered_spat[[2]]

# Add as new channel to Xenium object
vrImages(Xen_R1[["Assay1"]], channel = "H&E") <-
  vrImages(Xen_R1_image_reg, name = "main_reg", channel = "H&E")
```

We can now observe the new channels available for the Xenium assay:
...
**Comparison:** The automated workflow (`addXeniumHE()`) combines all these steps into a single function call, while the interactive workflow provides more control through the Shiny interface for fine-tuning alignment parameters.
